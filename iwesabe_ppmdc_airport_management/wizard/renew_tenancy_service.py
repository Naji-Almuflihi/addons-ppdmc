...
class WizardRenewService(models.TransientModel):
    _name = 'renew.tenancy.service'
    _description = 'Renew Tenancy service'
    ...
    def renew_contract(self):
        ...
        modid = self.env.ref(
            'iwesabe_ppmdc_airport_management.tenancy_service_form').id
        if context.get('active_ids', []):
            for rec in self:
                ...
                act_prop = self.env['tenancy.service'].browse(
                    context['active_ids'])
                act_prop.write({
