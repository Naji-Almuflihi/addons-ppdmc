...
class TenancyPropertyReport(models.TransientModel):
    ...
    def open_tenancy_by_property_gantt(self):
        ...
        wiz_form_id = self.env.ref(
            'iwesabe_ppmdc_airport_management.view_analytic_gantt').id
        analytic_obj = self.env['tenant.tenancy']
        tenancy_ids = False
    ...
    def open_tenancy_by_property_tree(self):
        ...
        """
        wiz_form_id = self.env.ref(
            'iwesabe_ppmdc_airport_management.property_analytic_view_tree').id
        analytic_obj = self.env['tenant.tenancy']
        tenancy_ids = False
        for data_rec in self:
            data = data_rec.read([])[0]
    ...
    def print_report(self):
        partner_obj = self.env['property.property']
        for data_rec in self:
            data = data_rec.read([])[0]
            partner_rec = partner_obj.browse(data['property_id'][0])
            data.update({'property_name': partner_rec.name})
        return self.env.ref(
            'iwesabe_ppmdc_airport_management.action_report_tenancy_by_property').report_action(
            [], data={
                'ids': self.ids,
