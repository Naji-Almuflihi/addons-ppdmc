...
class WizardRenewTenancy(models.TransientModel):
    _name = 'renew.tenancy.equipment'
    _description = 'Renew Tenancy Equipment'
    ...
    def renew_contract(self):
        ...
        modid = self.env.ref('iwesabe_ppmdc_custom.tenancy_equipment_form').id
        if context.get('active_ids', []):
            for rec in self:
                ...
                act_prop = self.env['tenancy.equipment'].browse(
                    context['active_ids'])
                act_prop.write({
