...
class AccontPaymentWizard(models.Model):
    ...
    amount_remain = fields.Monetary(string='Remaining Amount for Payment', store=True, readonly=True, compute='remain_amount_')
    amount_due_remain = fields.Monetary(string='Remaining Amount for Invoice', store=True, readonly=True, compute='remain_amount_')
    ...
    # po_number = fields.Many2many('purchase.order', 'po_multi_payment_line', string="PO Number")
    po_number = fields.Many2many(string="PO Number", related="payment_id.po_number")

    def partial_pay(self):
        for payment in self:
            payment = payment.with_context(partial_pay=True)
            active_id = payment.move_id
            other_move_line_id = payment.move_line_id
            active_move_line_id = active_id.line_ids.filtered(lambda x:x.account_id.user_type_id.type in ('receivable', 'payable'))
            lines = other_move_line_id + active_move_line_id
            currency_amount_to_pay = active_id.currency_id._convert(payment.amount_to_pay, payment.company_currency_id, active_id.company_id, active_id.date or fields.Date.context_today(self))
            lines.with_context(
                other_move_line_id = other_move_line_id.id,
                active_move_line_id = active_move_line_id.id,
                currency_amount_to_pay = currency_amount_to_pay,
    ...
    #             if payment.move_id.is_inbound():
    #                 lines = payment_move_id_with_ctx.line_ids.filtered(lambda x: (x.credit > 0 and x.debit == 0) and (x.partner_id == partner_id) and not x.reconciled and x.last_line_number == do_last_number)
    #             else:
    #                 lines = payment_move_id_with_ctx.line_ids.filtered(lambda x: (x.credit == 0 and x.debit > 0) and (x.partner_id == partner_id) and not x.reconciled and x.last_line_number == do_last_number)
    ...
    #         if payment_move_id.state == 'draft':
