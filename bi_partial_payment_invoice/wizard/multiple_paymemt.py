...
class AccontMultiPaymentWizard(models.Model):
            ...
            if payment.move_lines_ids:
                for line in payment.move_lines_ids:
                    if transfer_currency_id:
                        ...
                        if line.is_comapany:
                            converted_amount = currency_id._convert(line.curr_amount_to_pay, company_currency_id, payment.company_id, fields.Date.context_today(self))
    ...
    payment_id = fields.Many2one('account.payment', 'Customer/Venodr Payment', related='move_line_id.payment_id', store=True)
    ...
    po_number = fields.Many2many(string="PO Number", related="payment_id.po_number")
    ...
    def multi_partial_pay(self):
        for payment in self:
            ...
            payment_move_id_with_ctx = payment_move_id.with_context(check_move_validity=False, skip_account_move_synchronization=True)
            if line_ids:
                line_ids = list(set(line_ids))
                payment_move_id_with_ctx.write({'line_ids': [(2, line) for line in line_ids]})
            ...
            for line in payment.move_lines_ids:
                ...
                if len(do_payment_move_vals) >= 1:
                    for vals in do_payment_move_vals:
                        ...
                        payment_move_id_with_ctx.line_ids.create(vals)

                    if line.move_id.is_inbound():
                        lines = payment_move_id_with_ctx.line_ids.filtered(lambda x: (x.credit > 0 and x.debit == 0) and (x.partner_id == partner_id) and not x.reconciled)
                    else:
                        lines = payment_move_id_with_ctx.line_ids.filtered(lambda x: (x.credit == 0 and x.debit > 0) and (x.partner_id == partner_id) and not x.reconciled)
            ...
            if remaining_payment:
                last_line_number += 1
                self.env.user.company_id.write({'last_line_number': last_line_number})
                ...
                self.env.user.company_id.write({'last_line_number': last_line_number})
            payment.payment_id.get_po_number()
...
class MultiMoveLine(models.Model):
    ...
    company_currency_id = fields.Many2one('res.currency', string="Company Currency", related='company_id.currency_id', help='Utility field to express amount currency')
    currency_id = fields.Many2one('res.currency', string="Currency", related='move_id.currency_id', readonly=True, help='Utility field to express amount currency')
    payment_type = fields.Selection(related='multi_payment_id.payment_type', string="Payment Type", store=True, readonly=True)
    amount_total = fields.Monetary(string='Amount Total', store=True, related="move_id.amount_total", default=0.00)
    amount_residual = fields.Monetary(string='Residual Amount', readonly=True, default=0.00, currency_field='company_currency_id')
    amount_residual_currency = fields.Monetary(string='Currency Residual Amount', readonly=True, default=0.00, currency_field='currency_id')
    ...
    payment_curr_id = fields.Many2one('res.currency', string="Payment Currency", related='multi_payment_id.currency_id')
        ...
        for sale in self:
            ...
            if not sale.is_comapany:
                ...
                if sale.currency_id != sale.company_id.currency_id:
                    sale.curr_amount_to_pay = company_currency_id._convert(sale.amount_to_pay, payment_curr_id, company_id, fields.Date.context_today(self))
        ...
        for sale in self:
            ...
            if sale.is_comapany:
                if company_id and company_currency_id:
                    converted_amount = payment_curr_id._convert(sale.curr_amount_to_pay, company_currency_id, company_id, fields.Date.context_today(self))
