...
class AccountMoveInherit(models.Model):
    ...
    def _check_balanced(self):
        if self._context.get('partial_pay'):
            return 
        else:
            return super()._check_balanced()

    def action_post(self):
        self = self.with_context(partial_pay=True)
    ...
    def compute_user_date(self):
        for record in self:
            ...
            record.update({'report_date': today, 'report_user_id': user})
...
class AccountMoveLineInherit(models.Model):
    ...
    def check_full_reconcile(self):
        ...
        currency = set([a.currency_id for a in amls if not a.currency_id.id])
        ...
        if self._context.get('partner_id') and self._context.get('partner_type'):
            ...
            if self._context.get('partner_type') == 'customer':
                ...
                args.extend([('credit', '>', 0), ('debit', '=', 0),
                             ('account_id', 'in', account_ids.ids or []), ('partner_id', '=', partner_id)])
            else:
                account_ids = self.env['account.account'].search([
                                ('company_id', '=', self.env.user.company_id.id),
    ...
    def _partial_reconcile(self):
        ...
        vals = sorted_lines._prepare_reconciliation_partials()

        if self._context.get('partial_pay'):
            other_move_line_id = self._context.get('other_move_line_id')
            active_move_line_id = self._context.get('active_move_line_id')
            currency_amount_to_pay = self._context.get('currency_amount_to_pay') 
            amount_to_pay = self._context.get('amount_to_pay')
            for val in vals:

                if val.get('credit_move_id') == active_move_line_id:
                    val['credit_amount_currency'] = amount_to_pay
                    val['debit_amount_currency'] = currency_amount_to_pay
                    val['amount'] = currency_amount_to_pay
                elif val.get('debit_move_id') == active_move_line_id:
                    val['credit_amount_currency'] = currency_amount_to_pay
                    val['debit_amount_currency'] = amount_to_pay
                    val['amount'] = currency_amount_to_pay
        partials = self.env['account.partial.reconcile'].create(vals)
    ...
    def unlink(self):
        for reconcile in self:
            if self._context.get('from_js'):
                reconcile = reconcile.with_context(partial_pay=True)
            #     print('self_context========>',self._context,reconcile)
            #     if self._context.get('move_id'):
            #         current_move_id = self.env['account.move'].browse(self._context.get('move_id'))
            #         if current_move_id.exists():
            #             move_line_ids = reconcile.credit_move_id + reconcile.debit_move_id
            #             other_line_id = move_line_ids.filtered(lambda x:x.move_id != current_move_id)
                        
            #             if other_line_id:
            #                 other_move = other_line_id.move_id
            #                 other_line_id.with_context(from_js=False).sudo().unlink()
            #                 if other_move.state == 'draft':
            #                     other_move.with_context(from_js=False).action_post()
            #         return super(AccountPartialReconcileInherit, self).unlink()
                ...
                #             current_move_id.with_context(check_move_validity=False).write({'line_ids': [(2, line) for line in remove_lines]})
