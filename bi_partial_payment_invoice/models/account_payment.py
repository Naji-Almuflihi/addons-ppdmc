...
class AccontPayment(models.Model):
    ...
    po_number = fields.Many2many('purchase.order', 'po_payment_rel', 'po_number', 'payment_id', string="PO Number", compute="get_po_number")

    # @api.depends('reconciled_bill_ids', 'state', 'move_id')
    def get_po_number(self):
        for res in self:
            res.po_number = []
            po_list = []
            for inv in res.reconciled_bill_ids:
                for inv_line in inv.invoice_line_ids:
                    if inv_line.purchase_order_id and inv_line.purchase_order_id.id not in po_list:
                        po_list.append(inv_line.purchase_order_id.id)
            if len(po_list) > 0:
                res.po_number = [(6, 0, po_list)]
