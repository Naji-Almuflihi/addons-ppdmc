odoo.define('iwesabe_general_ledger.test', function (require) {
    'use strict';
    ...
    account_report_generic.include({
        ...
        'value_changed': function(ev) {
             ...
             self.report_options.supplier_site_ids = ev.data.supplier_site_ids;

             return self.reload().then(function () {
                 self.$searchview_buttons.find('.account_partner_filter').click();
                 self.$searchview_buttons.find('.account_group_id_filter').click();
                 self.$searchview_buttons.find('.account_analytic_filter').click();
                 self.$searchview_buttons.find('.account_supplier_site_filter').click();
             });
            },
        },
...
//            this._super.apply(this, arguments);
            ...
            // Supplier Site filter
            if (this.report_options.supplier_site_ids) {
//                if (!this.M2MFilters) {
                    var fields = {};
                    if ('supplier_site_ids' in this.report_options) {
                        fields['supplier_site_ids'] = {
                            label: _t('Supplier Site'),
                            modelName: 'site.solution',
                            value: this.report_options.supplier_site_ids.map(Number),
                        };
                    }

                    if (!_.isEmpty(fields)) {
                        this.M2MFilters = new M2MFilters(this, fields);
                        this.M2MFilters.appendTo(this.$searchview_buttons.find('.js_account_supplier_site_m2m'));
                    }
            }
