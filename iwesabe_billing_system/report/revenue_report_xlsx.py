...
class revenue_reportXls(models.AbstractModel):
    _name = 'report.iwesabe_billing_system.revenue_report_xlsx'
    ...
    def generate_xlsx_report(self, workbook, data, lines):
        ...
        row = 0
        col = -1
        worksheet.merge_range('A3:E3', 'Report For Revenue From ' + str(lines.from_date) + ' To ' + str(lines.to_date), f1)
        ...
        for part in partners:
            ...
            for app_part in appendix_partner:
                gaca_tbc_total += sum(app_part.invoice_appendix_line_ids.mapped('pax')) * gaca_tpc if gaca_tpc else sum(app_part.invoice_appendix_line_ids.mapped('pax'))
                term_facilities_utilization_total += app_part.term_facilities_utilization_total
                systems_total += app_part.systems_total
                field_400_hz_total += app_part.field_400_hz_total
                ground_handling_total += app_part.ground_handling_total
                aircraft_parking_not_registered_total += app_part.aircraft_parking_not_registered_total
                aircraft_registered_total += app_part.aircraft_registered_total
                plb_busses_total += app_part.plb_busses_total
                security_services_total += app_part.security_services_total
                fees_total_total += app_part.fees_total_total
            row += 1
            seq += 1
