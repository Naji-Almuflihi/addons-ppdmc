...
class GlobalDiscountInvoice(models.Model):
    ...
    global_discount_type = fields.Selection([
        ...
        states={'draft': [('readonly', False)],
                'sent': [('readonly', False)]},
        default='percent')
    global_discount_rate = fields.Float('Universal Discount', readonly=True, states={'draft': [('readonly', False)], 'sent': [('readonly', False)]})
    amount_discount = fields.Monetary(string='Universal Discount', readonly=True, compute='_compute_amount', store=True, track_visibility='always')
    enable_discount = fields.Boolean(compute='verify_discount_discount')
    sales_discount_account_id = fields.Integer(compute='verify_discount_discount')
    purchase_discount_account_id = fields.Integer(compute='verify_discount_discount')

    @api.depends('company_id.enable_discount')
    def verify_discount_discount(self):
        for rec in self:
    ...
    def _prepare_refund(self, invoice, date_invoice=None, date=None, description=None, journal_id=None):
        ks_res = super(GlobalDiscountInvoice, self)._prepare_refund(invoice, date_invoice=None, date=None, description=None, journal_id=None)
                    ...
                    for record in terms_lines:
                        if rec.global_discount_type == "percent":
                            record.update({
                                'amount_currency': -total_amount_currency,
                                'debit': (record.debit - ((
                                                                  record.debit * self.global_discount_rate) / 100)) if total_balance < 0.0 else 0.0,
                                'credit': (record.credit - ((
                                                                    record.credit * self.global_discount_rate) / 100)) if total_balance > 0.0 else 0.0
                            })
                        else:
        ...
        for rec in self:
            ...
            if rec.global_discount_rate > 0 and rec.move_type in type_list:
                if rec.is_invoice(include_receipts=True):
                    ...
                    dis_value = ""
                    if rec.global_discount_type == "amount":
                        dis_value = "of amount #" + str(self.global_discount_rate)
                    elif rec.global_discount_type == "percent":
                        dis_value = " @" + str(self.global_discount_rate) + "%"
                    else:
                        dis_value = ''
                    dis_name = dis_name + dis_value
