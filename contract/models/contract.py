...
class ContractContract(models.Model):
    ...
    state = fields.Selection(string="Status", selection=[('draft', 'Draft'), ('confirmed', 'Confirmed'),('terminated','Terminated') ], default='draft' )

    has_accountant_group = fields.Boolean(compute='compute_has_accountant_group')
    ...
    @api.depends('contract_line_fixed_ids')
    def compute_total_amount(self):
        for rec in self:
            ...
            if rec.contract_line_fixed_ids:
                for line in rec.contract_line_fixed_ids:
    ...
    def compute_has_accountant_group(self):
        for rec in self:
            if self.env.user.has_group("account.group_account_user"):
                rec.has_accountant_group=True
            else:
                rec.has_accountant_group = False
