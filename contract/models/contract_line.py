...
class ContractLine(models.Model):
    ...
    contract_id = fields.Many2one("contract.contract", string="Contract", required=True, index=True, auto_join=True, ondelete="cascade")
    analytic_account_id = fields.Many2one("account.analytic.account", string="Analytic account")
    analytic_tag_ids = fields.Many2many("account.analytic.tag", string="Analytic Tags")
    date_start = fields.Date(required=True)
    date_end = fields.Date(compute="_compute_date_end", store=True, readonly=False)
    termination_notice_date = fields.Date(string="Termination notice date", compute="_compute_termination_notice_date", store=True)
    create_invoice_visibility = fields.Boolean(compute="_compute_create_invoice_visibility")
    successor_contract_line_id = fields.Many2one("contract.line", string="Successor Contract Line",  readonly=True, index=True, copy=False)
    predecessor_contract_line_id = fields.Many2one("contract.line", string="Predecessor Contract Line", readonly=True, index=True, copy=False)
    ...
    active = fields.Boolean(string="Active", related="contract_id.active", store=True, default=True)

    @api.depends("last_date_invoiced", "date_start", "date_end", "contract_id.last_date_invoiced")
    def _compute_next_period_date_start(self):
        ...
        for rec in self:
            ...
            if not rec.last_date_invoiced and any(lines.mapped("last_date_invoiced")):
                next_period_date_start = max(lines.filtered("last_date_invoiced").mapped("last_date_invoiced")) + relativedelta(days=1)
    ...
    def _compute_termination_notice_date(self):
        for rec in self:
            if rec.date_end:
                rec.termination_notice_date = rec.date_end - self.get_relative_delta(rec.termination_notice_rule_type, rec.termination_notice_interval)
            else:
    ...
    def _get_state_domain(self, state):
        ...
        if state == "in-progress":
            return ["&", "&", "&", ("date_start", "<=", today), ("is_canceled", "=", False), "|", ("date_end", ">=", today), ("date_end", "=", False), "|", ("is_auto_renew", "=", True), "&", ("is_auto_renew", "=", False), ("termination_notice_date", ">", today)]
        if state == "to-renew":
            return ["&", "&", ("is_canceled", "=", False), ("date_end", "<", today), "|", "&", ("manual_renew_needed", "=", True), ("successor_contract_line_id", "=", False), ("is_auto_renew", "=", True)]
        if state == "upcoming-close":
            return ["&", "&", "&", "&", "&", ("date_start", "<=", today), ("is_auto_renew", "=", False), ("manual_renew_needed", "=", False), ("is_canceled", "=", False), ("termination_notice_date", "<", today), ("date_end", ">=", today)]
        if state == "closed":
            return [
                "&", "&", "&", ("is_canceled", "=", False), ("date_end", "<", today), ("is_auto_renew", "=", False),
                "|", "&", ("manual_renew_needed", "=", True), ("successor_contract_line_id", "!=", False), ("manual_renew_needed", "=", False),
            ]
        if state == "canceled":
    ...
    def _compute_allowed(self):
        for rec in self:
            ...
            if rec.date_start:
                ...
                if allowed:
                    rec.update(
                        {"is_plan_successor_allowed": allowed.plan_successor, "is_stop_plan_successor_allowed": (allowed.stop_plan_successor),
    ...
    @api.model
    def _compute_first_recurring_next_date(self, date_start, recurring_invoicing_type, recurring_rule_type, recurring_interval):
