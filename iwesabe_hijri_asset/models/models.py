...
class HijriFiledEditWizard(models.TransientModel):
    _name = 'hijri.field.edit.wizard'

    day = fields.Selection(DAYS, default=lambda x: x.get_default_date()['day'])
    month = fields.Selection(HIJRI_MONTHS, string='Month', default=lambda x: x.get_default_date()['month'])
    year = fields.Selection(YEARS, default=lambda x: x.get_default_date()['year'])

    def get_default_date(self):
        if not self._context.get('date_field'):
            return {'day': False, 'month': False, 'year': False}

        field = self._context['date_field']
        model = self._context['active_model']
        active_id = self._context['active_id']
        parent_id = self.env[model].browse(active_id)

        field_val = parent_id.read([field])[0][field]

        if not field_val:
            return {'day': False, 'month': False, 'year': False}

        day = str(split_hijri(field_val)['day']).zfill(2)
        month = split_hijri(field_val)['month']
        year = split_hijri(field_val)['year']

        if STORE_HIJRI_ARABIC:
            day = arabic_2_num(day)
            year = arabic_2_num(year)
            ...
            hijri_months = {
                ...
                'ربيع الاول': 'Rabi-I',
                'ربيع الاخر': 'Rabi-II',
                'جمادي الاول': 'Jumada-I',
                'جمادي الاخر': 'Jumada-II',
                'رجب': 'Rajab',
                'شعبان': 'Shaban',
                'رمضان': 'Ramadan',
                'شوال': 'Shawwal',
                'ذو القعده': 'Dhul-Qadah',
                'ذو الحجة': 'Dhul-Hijjah',
            }

            month = hijri_months[split_hijri(field_val)['month']]
    ...
    def write_date_in_field(self):
        ...
        active_id = self._context['active_id']
        parent_id = self.env[model].browse(active_id)
        lst = [bool(self.day), bool(self.month), bool(self.year)]

        if not all(lst):
            raise UserError('Invalid Date  ..!!')

        d = format_date(self.day, dict(HIJRI_MONTHS)[self.month], self.year)
        parent_id.write({field: d})

    def delete_date_from_field(self):
        field = self._context['date_field']
        model = self._context['active_model']
        active_id = self._context['active_id']
        parent_id = self.env[model].browse(active_id)
        parent_id.write({field: False})
