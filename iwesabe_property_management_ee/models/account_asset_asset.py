...
class AccountAssetAsset(models.Model):
    ...
    def open_assets(self):
        for asset in self:
            res = self.env['account.asset'].search(
                [('property_id', '=', asset.id)])
        ctx = {'default_asset_type': 'expense'}
        ctx.update({'default_property_id': self.id})
        list_id = self.env.ref('account_asset.view_account_asset_purchase_tree').id
        form_id = self.env.ref('account_asset.view_account_asset_form').id
        asset_type = "expense"
        return {
            'name': _('Account Asset'),
            'view_type': 'form',
            'view_mode': 'tree,form',
            'res_model': 'account.asset',
            'view_id': False,
            # 'view_ids': [(list_id), (form_id)],
            'type': 'ir.actions.act_window',
            'domain': [('id', 'in', res.ids)],
            'context': ctx,
            'views': self.env['account.asset']._get_views(asset_type),
        }
    ...
    property_id = fields.Many2one('account.asset')
