...
class ResPartner(models.Model):
    ...
    def write(self, vals):
        ...
        tenant_group = \
            self.env.ref('iwesabe_property_management_ee.group_property_user')
        agent_group = \
            self.env.ref('iwesabe_property_management_ee.group_property_agent')
        for partner in self:
            if 'tenant' in vals:
                if not partner.tenant:
                    if partner.user_ids.has_group(
                            'iwesabe_property_management_ee.group_property_user'):
            ...
            if 'agent' in vals:
                if not partner.agent:
                    if partner.user_ids.has_group(
                            'iwesabe_property_management_ee.group_property_agent'):
                        partner.user_ids.write(
