...
class TenancyPropertyReport(models.TransientModel):
    ...
    def open_tenancy_by_property_gantt(self):
        ...
        wiz_form_id = self.env.ref(
            'iwesabe_property_management_ee.view_analytic_gantt').id
    ...
    def open_tenancy_by_property_tree(self):
        ...
        """
        wiz_form_id = self.env.ref(
            'iwesabe_property_management_ee.property_analytic_view_tree').id
        analytic_obj = self.env['account.analytic.account']
        tenancy_ids = False
        for data_rec in self:
            data = data_rec.read([])[0]
    ...
    def print_report(self):
        ...
        return self.env.ref(
            'iwesabe_property_management_ee.action_report_tenancy_by_property').report_action(
            [], data={
                'ids': self.ids,
                'model': 'account.asset',
                'form': data
